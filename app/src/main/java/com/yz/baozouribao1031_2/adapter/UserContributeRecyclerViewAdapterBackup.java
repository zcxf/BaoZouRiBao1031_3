...
public class UserContributeRecyclerViewAdapterBackup extends
        RecyclerViewAdapterHelper<UserContributeBean.DataBean> {

    private static final int TYPE_ITEM = 0;
    private static final int TYPE_FOOTER = 1;

    public UserContributeRecyclerViewAdapterBackup(Context context, List<UserContributeBean.DataBean>
            list) {
        super(context, list);
    }
    ...
    public RecyclerView.ViewHolder onCreateMyViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = mInflater.inflate(R.layout.item1_recyclerview_home_first_page, parent,
                    false);
    ...
    public void onBindMyViewHolder(RecyclerView.ViewHolder holder, int position) {
        UserContributeBean.DataBean dataBean = mList.get(position);
        ...
        //拼接的用户投稿网址
        final String finalUrlString = URLConstants.URL_USER_CONTRIBUTE_DETAIL + mList.get
                (position).getDocument_id();
